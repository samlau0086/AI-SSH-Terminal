...
- **完善的会话配置**: 提供 SSH 连接会话的新增、编辑、删除（带确认提示）以及按标签分类，支持密码/私钥统一凭证管理 (Credential Manager)，可在新建会话时快速加载。支持设置会话到期日及自动续费周期提醒。
- **跳板机 / Bastion Host 支持**: 每个 VPS 会话都可以选择一个已保存的会话作为跳板机。终端、批量命令、SFTP 文件管理、系统状态面板和在线监控都会自动通过跳板机建立 SSH 隧道，适合将目标 VPS 的 SSH 端口只放行给特定安全入口机器。
- **系统监控与自动告警**: 实时展示所在系统的 CPU 利用率、内存分布情况、网络上下行负载状态。内置后台监控守护进程，支持断网离线报警（基于 Bark）及 VPS 到期预警功能。
...
### 使用跳板机 / Bastion Host
跳板机适合更严格的安全策略：目标 VPS 不再对公网或管理端直接暴露 SSH，只允许来自某一台可信机器（跳板机）的 SSH 连接。

1. 先把跳板机本身创建为一个普通 SSH 会话，例如 `bastion-01`。
2. 再创建或编辑目标 VPS 会话，在 `Jump Host` 下拉框中选择刚才保存的跳板机会话。
3. 保存目标 VPS 会话后，正常打开终端即可。系统会先连接跳板机，再从跳板机通过 SSH 隧道连接目标 VPS。
4. 在目标 VPS 的防火墙或云安全组中，将 SSH 端口（通常是 22）限制为只允许跳板机的内网 IP 或公网 IP 访问。

当前版本支持一层跳板机，不支持“跳板机再走另一个跳板机”的多级链路。跳板机本身建议使用独立密钥、最小权限用户，并开启云厂商安全组 / 防火墙白名单。

### 会话间文件传输
1. 至少保存两个 SSH 会话，并打开源服务器的文件管理面板。
2. 在文件或目录所在行点击双向箭头按钮，选择目标会话和目标目录。
3. 点击“开始传输”。目录会递归复制；如目标存在同名文件，确认后才会覆盖。
4. 传输数据直接从源服务器的 SFTP 读流写入目标服务器，不经过浏览器或应用服务器临时文件。传输过程中可查看文件和字节进度，也可随时取消。

会话间传输兼容跳板机。当前支持普通文件和目录，不复制符号链接或其他特殊文件；取消时已完成的文件会保留，正在写入的临时文件会被清理。
